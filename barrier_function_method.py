...
def get_helper_function(fn : FuncWithGradient, restriction_functions, rk) -> FuncWithGradient:
    return FuncWithGradient(lambda x: fn.get_func(x) - rk * sum(map(lambda g: np.log(-g.get_func(x)), restriction_functions)),
                            lambda x: fn.get_gradient(x) - rk * sum(map(lambda g: g.get_gradient(x) / g.get_func(x), restriction_functions)))

def is_method_finish(point, rk, restriction_functions, eps = 0.01) -> bool:
    P = -rk * sum(map(lambda g: np.log(-g.get_func(point)), restriction_functions))
...
def barrier_function_method(func : FuncWithGradient, restriction_functions, start_point, r0, C):
    ...
    while is_continue:
        ...
        best_point = fast_descent_method(helper_func, now_point)
